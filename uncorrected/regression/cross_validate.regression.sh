#!/bin/bash
CELL_LINE=$1
echo "$CELL_LINE/predictions.DNASE.$CELL_LINE.regressionlabels"
[ -d $CELL_LINE ] || mkdir $CELL_LINE
CUDA_VISIBLE_DEVICES=1 kerasAC_cross_validate --index_data_path /srv/scratch/annashch/bias_correction/genomewide_labels/regressionlabels.allbins.hg38.hdf5 \
		    --input_data_path seq \
		    ...
		    --num_inputs 1 \
		    --num_outputs 1 \
		    --model_hdf5 $CELL_LINE/DNASE.$CELL_LINE.regressionlabels \
		    ...
		    --patience_lr 2 \
		    --expand_dims \
		    --predictions_and_labels_hdf5 $CELL_LINE/predictions.DNASE.$CELL_LINE.regressionlabels \
		    --performance_metrics_regression_file $CELL_LINE/performance.DNASE.$CELL_LINE.regressionlabels \
		    --tasks $CELL_LINE \
