...
def getModelGivenModelOptionsAndWeightInits(args):
    ...
    if 'conv2_kernel_size' in model_params:
        conv2_kernel_size=int(model_params['conv2_kernel_size'])
    if 'conv3_kernel_size' in model_params:
        conv3_kernel_size=int(model_params['conv3_kernel_size'])
    ...
    first_conv = Conv1D(filters,
                        ...
                        activation='relu',
                        name='1st_conv')(inp)
    second_conv=Conv1D(filters,
                       kernel_size=conv2_kernel_size,
                       padding='valid',
                       activation='relu',
                       name='2nd_conv')(first_conv) 
    third_conv=Conv1D(filters,
                      kernel_size=conv3_kernel_size,
                      padding='valid',
                      activation='relu',
                      name='3rd_conv')(second_conv) 

    profile_out_prebias_shape =int_shape(third_conv)
    ...
    profile_out_prebias = Cropping1D((crop_left,crop_right),
                                     name='prof_out_crop2match_output')(third_conv)
    profile_out = Conv1D(filters=num_tasks,
                         kernel_size=1,
                         name="profile_predictions")(profile_out_prebias)
    gap_combined_conv = GlobalAveragePooling1D(name='gap')(third_conv)
