...
def parse_args():
    ...
    parser.add_argument("-prediction_kmer_scores")
...
def main():
    args=parse_args()
    prediction_kmer_scores=open(args.prediction_kmer_scores,'r').read().strip().split('\n')
    prediction_kmer_dict={}
    for line in prediction_kmer_scores:
        tokens=line.split('\t') 
        prediction_kmer_dict[tokens[0]]=float(tokens[1])
    print("loaded kmer dict") 
    ...
    for kmer in kmer_dict_profile:
        pwm_profile+=(prediction_kmer_dict[kmer]*kmer_dict_profile[kmer])
        pwm_counts+=(prediction_kmer_dict[kmer]*kmer_dict_counts[kmer])
